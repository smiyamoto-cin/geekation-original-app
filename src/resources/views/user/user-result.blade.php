...
@if ($nextQuiz)
  <a href="{{ route('quiz.next', ['category_id' => $category->id, 'title_id' => $title->id, 'quiz_id' => $quiz->id])}}" class="btn btn-success">次の問題へ</a>
@else
  <a href="{{ route('quiz.next', ['category_id' => $category->id, 'title_id' => $title->id, 'quiz_id' => $quiz->id]) }}" class="btn btn-success">結果を見る</a>
@endif
